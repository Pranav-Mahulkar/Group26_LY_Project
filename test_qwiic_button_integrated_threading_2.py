...
def select_channel(channel):
    ...
    time.sleep(0.001)  # Reduced delay to speed up data collection
...
def read_imu_data(imu):
    try:
        ...
        ax_mps2 = ax * GRAVITY  # Convert to m/s²
        ay_mps2 = ay * GRAVITY
        az_mps2 = az * GRAVITY
...
def main():
    with SMBus(1) as bus:
        ...
        try:
            ...
            headers = ["Timestamp", "Sensor", "Accel X (m/s²)", "Accel Y (m/s²)", "Accel Z (m/s²)", "Gyro X (°/s)", "Gyro Y (°/s)", "Gyro Z (°/s)"]
            ...
            while True:
                ...
                if collecting_data:
                    ...
                    # Save each sensor's data in a separate row with sensor label
                    ...
                    # Adjust sleep to maintain 100 Hz rate (0.01 seconds)
                    elapsed_loop_time = time.time() - loop_start_time
                    time_to_sleep = max(0, 0.01 - elapsed_loop_time)  # Ensure 100 Hz
