...
def get_quaternion_from_imu(imu):
    """
    Reads accelerometer and gyroscope data from the IMU and calculates quaternion.
    This is an example and assumes your IMU library provides methods to read raw data.

    :param imu: IMU object (sensor instance)
    :return: Quaternion as a tuple (q0, q1, q2, q3)
    """
    try:
        # Read accelerometer data
        accel_x, accel_y, accel_z = read_accel(imu)  # Replace with actual IMU library calls
        
        # Read gyroscope data
        gyro_x, gyro_y, gyro_z = read_gyro(imu)  # Replace with actual IMU library calls

        # Example: Calculate a simple quaternion (stub for demo purposes)
        # Replace this with a real algorithm like Madgwick/Mahony filter
        q0 = math.sqrt(1 - accel_x**2 - accel_y**2 - accel_z**2) if accel_x**2 + accel_y**2 + accel_z**2 <= 1 else 0
        q1 = accel_x
        q2 = accel_y
        q3 = accel_z
        ...
        raise RuntimeError(f"Failed to get quaternion from IMU: {e}")
...
def rotate(acceleration, quaternion):
    """
    Rotate a vector (acceleration) by a quaternion into the Earth's frame of reference.
    :param acceleration: List or numpy array of accelerometer readings [ax, ay, az].
    :param quaternion: List or numpy array representing the quaternion [qx, qy, qz, qw].
    :return: Rotated acceleration vector.
    """
    # Convert quaternion to a rotation object
    rotation = R.from_quat(quaternion)
    # Apply rotation to the acceleration vector
    return rotation.apply(acceleration)

def calculate_user_acceleration(acceleration, quaternion):
    """
    Calculate the linear acceleration by rotating the accelerometer reading and subtracting gravity.
    :param acceleration: Raw accelerometer data [ax, ay, az].
    :param quaternion: Quaternion representing orientation [qx, qy, qz, qw].
    :return: Linear acceleration vector (user acceleration).
    """
    a_rotated = rotate(acceleration, quaternion)
    user_acceleration = a_rotated - np.array([0, 0, -GRAVITY])
    return user_acceleration


def read_imu_data(imu, accel_offsets=None, gyro_offsets=None, quaternion=None):
    try:
        ...
        # Assume quaternion is provided from IMU API
        if quaternion:
            linear_acceleration = calculate_user_acceleration([ax, ay, az], quaternion)
        else:
        ...
        if accel_offsets:
            linear_acceleration = [la - offset for la, offset in zip(linear_acceleration, accel_offsets)]
        if gyro_offsets:
            gx -= gyro_offsets[0]
            gy -= gyro_offsets[1]
            gz -= gyro_offsets[2]

        return [*linear_acceleration, gx, gy, gz]
    except pigpio.error as e:
        return [f"Error: {e}"] * 6
        ...
        try:
            ...
            while True:
                ...
                if collecting_data:
                    ...
                    for idx, imu in enumerate(imu_sensors):
                        select_channel(idx + 1)  # IMUs are on channels 1, 2, and 3
                        try:
                            raw_data = read_imu_data(imu, accel_offsets[idx], gyro_offsets[idx])
                            
                            # Apply low-pass filters to gyroscope data
                            filtered_gyro = [
                                imu_filters[idx][i].filter(raw_data[3 + i]) for i in range(3)
                            ]
                            
                            # Save data to CSV
                            data_row = [elapsed_time, f"IMU {idx + 1}", *raw_data[:3], *filtered_gyro]
                            save_to_csv(data_row, filename)
                        except Exception as e:
                            print(f"Error collecting data from IMU {idx + 1}: {e}")
                    
                    time.sleep(0.01)  # Maintain a consistent sample rate (~100 Hz)
                    ...
                    for i, imu in enumerate(imu_sensors, start=1):
                        quaternion = get_quaternion_from_imu(imu)
                        print(f"Quaternion: {quaternion}")

                        thread = threading.Thread(
                            target=collect_sensor_data, 
                            args=(i, imu, filename, data_container, quaternion)
                        )
